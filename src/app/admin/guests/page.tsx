...
import { getGuestRows } from "@/db/queries/admin";
import { requireAdmin } from "@/lib/auth";
import { parseGuestView, parseStatusFilter } from "@/lib/admin-filters";
import { statusLabels } from "@/lib/format";
import { CreateGuestForm, GuestCard } from "@/components/admin/GuestForms";
...
export default async function AdminGuestsPage({
  ...
  const guests = await getGuestRows(view, status);
  ...
  return (
    <main className={`shell ${styles.adminPage}`}>
      ...
      <section className={styles.guestList} style={{ marginTop: 16 }}>
        {guests.length === 0 ? (
          ...
          guests.map((guest) => (
            <GuestCard key={guest.id} guest={guest} adminSide={admin.side} />
          ))
        )}
      </section>
    </main>
